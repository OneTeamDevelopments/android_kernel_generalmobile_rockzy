...
static int bcm2079x_parse_dt(struct device *dev, struct bcm2079x_platform_data *platform_data)
{
	struct device_node *np = dev->of_node;
	platform_data->irq_gpio = of_get_named_gpio_flags(np, "broadcom,irq_gpio",
				0, NULL);
	platform_data->en_gpio = of_get_named_gpio_flags(np, "broadcom,en_gpio",
				0, NULL);
 	platform_data->wake_gpio = of_get_named_gpio_flags(np, "broadcom,wake_gpio",
				0, NULL);
	return 0;
}
