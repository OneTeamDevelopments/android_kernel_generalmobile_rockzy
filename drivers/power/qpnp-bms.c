...
#ifdef CONFIG_GN_Q_BSP_PM_CHG_DEBUG_SUPPORT
#undef pr_debug                                                                                                                                                               
#define pr_debug(fmt, ...) \
				...
				int batt_temp)
{
	...
	if (chip->prev_last_good_ocv_raw == OCV_RAW_UNINITIALIZED) {
		...
		pr_info("PON_OCV_UV = %d, cc = %llx,ocv_raw = 0x%x\n",
				chip->last_ocv_uv, raw->cc,raw->last_good_ocv_raw);
		...
		chip->software_shdw_cc_uah = 0;
		chip->last_cc_uah = INT_MIN;
		pr_info("EOC Battery full ocv_reading = 0x%x,last_ocv_uv = %d\n",
				chip->ocv_reading_at_100,chip->last_ocv_uv);
...
static void load_shutdown_data(struct qpnp_bms_chip *chip)
{
	...
	pr_info("calculated_soc = %d, shutdown_soc = %d, valid_limit = %d\n",
			calculated_soc, shutdown_soc, chip->shutdown_soc_valid_limit);
